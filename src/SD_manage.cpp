...
SD_thread = freertos::thread::create([](void*){
    while(true){
        ...
        // Cast your custom queue to get the underlying FreeRTOS queue handle
        QueueHandle_t queueHandle = (QueueHandle_t)queue.get_handle(); // Add get_handle() if needed
        
        // Check if we have enough messages to process
        if(uxQueueMessagesWaiting(queueHandle) > 30) {
            fs::File file;
